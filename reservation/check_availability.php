...
if ($stmt === false) {
    echo "Error executing SQLcommand: " . print_r(sqlsrv_errors(), true);
    return;
}
...
// Print the available rooms as a JSON object
if (count($availableRooms) == 0) {
    // No available rooms found, return a JSON object with an appropriate message
    $availableRooms = array("message" => "No available rooms found for the selected dates.");
}

$availableRoomsJSON = json_encode($availableRooms);

// Output the JSON data as a string
echo $availableRoomsJSON;
